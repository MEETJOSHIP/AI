...
import {
  Shield, Search, Bell, Settings, TrendingUp,
  Radio, AlertTriangle, ShieldAlert, Cloud, Network, Cpu,
  ChevronRight, Circle, Code2, Sparkles, Boxes, BarChart3,
  FileText, LayoutDashboard, Crosshair, LogOut,
} from "lucide-react";
import {
  AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer,
} from "recharts";
import { useAuth } from "../contexts/AuthContext";
import { fetchIncidentsSummary } from "../services/api";
  ...
  useEffect(() => {
    ...
    function frame(ts) {
      ...
      const projected = points.map((p) => {
        ...
        return { sx: cx + rx * R, sy: cy + p.y * R, depth: rz, threat: p.threat };
      });
      ...
      if (ts - lastSpawnRef.current > 1400 && threatScreenPts.length > 1) {
        ...
        if (a !== b) arcsRef.current.push({ from: a, to: b, start: ts, dur: 1100 });
      }
...
export default function Dashboard() {
  ...
  const [realIncidents, setRealIncidents] = useState(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const data = await fetchIncidentsSummary(token);
        setRealIncidents(data);
      } catch {
        // backend unreachable mid-session - keep last known value
      }
    };
    load();
